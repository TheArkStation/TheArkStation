...
/obj/machinery/autodoc
	...
	var/list/queue = list()
	var/permitted = list(/obj/item/weapon/implant/chem, /obj/item/weapon/implant/death_alarm, /obj/item/weapon/implant/loyalty, /obj/item/weapon/implant/adrenalin, /obj/item/weapon/implant/translator)
	var/operating = 0
	var/obj/item/reagent_container/blood/OMinus/blood_pack
	var/obj/item/weapon/reagent_containers/glass/beaker/loaded
	var/stasis = 1
	var/anestesia = 0
	var/filtering = 0
	...
	if(Org in organs_ext)
		...
		switch(opname)
			...
			if("remove")
				...
				O.parent.take_external_damage(rand(5,14), used_weapon = /obj/item/weapon/circular_saw)
			...
			if("fix Bones")
				...
				if(chance > 8)
					O.take_external_damage(3, used_weapon = /obj/item/weapon/scalpel)
			...
			if("attach")
				...
				if(Stor_autodoc in typesof(/obj/item/organ/external) )
					...
					O.take_external_damage(rand(0,4), used_weapon = /obj/item/weapon/scalpel)
					sleep(30)
		...
		switch(opname)
			if("Heal")
				if(!O.surface_accessible)
					P.take_external_damage(rand(0,4), used_weapon = /obj/item/weapon/scalpel)
					occupant.custom_pain("[src.name] made a cut on your skin",10,affecting = O)
				while (O.damage > 0)
					sleep(30)
					O.damage -= rand(3,6)
			...
			if("necro")
				if(!O.surface_accessible)
					P.take_external_damage(rand(0,7), used_weapon = /obj/item/weapon/scalpel)
					occupant.custom_pain("[src.name] made a cut on your skin",10,affecting = O)
				sleep(rand(30,50))
				O.germ_level = 0
				O.status &= ~ORGAN_DEAD
...
/datum/nano_module/autodoc_console
	...
	var/list/permitted_implants = list(/obj/item/weapon/implant/chem, /obj/item/weapon/implant/death_alarm, /obj/item/weapon/implant/loyalty, /obj/item/weapon/implant/adrenalin, /obj/item/weapon/implant/translator)
	var/emagged = 0
	...
	if(connected)
		for(var/obj/item/organ/external/O in connected.organs_ext)
			...
			if(BP_IS_ROBOTIC(O))
				...
				switch(O.damage)
					if(1 to O.max_damage)
						status.Add(" damaged")
					if(O.max_damage to INFINITY)
			...
			for(var/obj/item/organ/internal/I in O.internal_organs)
				...
				if(BP_IS_ROBOTIC(I))
					...
					switch(O.damage)
						if(1 to I.max_damage)
							internalstatus.Add(" damaged")
						if(I.max_damage to INFINITY)
							internalstatus.Add(" seriously damaged")
